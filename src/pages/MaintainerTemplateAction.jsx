...
function MaintainerTemplateAction() {
  ...
  // Submit button sends all values to backend
  const handleSubmit = async () => {
    try {
      const token = localStorage.getItem("token");
      await axios.put(
        `http://localhost:8000/checklist/details/${id}/status`,
        { status, delay_reason: delayReason, notes },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      navigate(
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8 font-sans">
      ...
      <form className="bg-white rounded-2xl shadow-lg p-6 border border-gray-200 mb-8">
        <div className="mb-4">
