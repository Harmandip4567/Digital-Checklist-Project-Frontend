...
const ExistingTemplates: React.FC = () => {
  ...
  // Get query params for delete action
  ...
  useEffect(() => {
    if (action === "delete" && templateId) {
      removeExistingTemplate(parseInt(templateId));
      navigate("/admin/ExistingTemplates");
    }
  }, [action, templateId, navigate]);
  ...
  // Delete template
  const removeExistingTemplate = async (Template_id: number) => {
    try {
  ...
  return (
    <Layout>
      <div className="min-h-screen bg-gradient-to-tr from-gray-50 font-sans">
        <Headerfile title="Existing Templates"  />
