...
interface Template {
  ...
  created_at: string;
  status?: string;
  item_responses?: string; // stored as JSON string from backend
}

type ItemResponses = Record<number, string | boolean>;
...
function MaintainerTemplateDetails() {
  ...
  const [itemResponses, setItemResponses] = useState<ItemResponses>({});

  // Get delayReason and notes from query params if passed
  const query = new URLSearchParams(location.search);
  const delayReason = query.get("delayReason") || "";
    ...
    <Layout>
      <div className="min-h-screen bg-gray-50 p-8 font-sans">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-200 mb-8">
          ...
          <p className="text-sm text-gray-700 mt-2">
            <span className="font-medium">Delay Reason:</span> {delayReason}
          </p>
          <p className="text-sm text-gray-700 mt-2">
